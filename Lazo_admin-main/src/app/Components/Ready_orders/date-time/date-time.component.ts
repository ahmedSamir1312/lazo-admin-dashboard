...
export class DateTimeComponent implements OnInit {
  ...
  constructor(private orders:AppService ,
    public dialogRef: MatDialogRef<DateTimeComponent> ,private formbuilder:FormBuilder ,@Inject(MAT_DIALOG_DATA) public data: any,private service:AppService ){
      this.order=data?.item;
      this.dynamicTimePeriod();
    }
  ...
  dynamicTimePeriod() {
    this.service.loadDeliveryData().subscribe((res: any) => {
      console.log("dynamicTimePeriod",res?.data);
      this.types=res?.data
    });
  }
