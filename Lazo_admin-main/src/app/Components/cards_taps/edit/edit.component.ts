...
export class EditComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.form = this.formbuilder.group({
      name_en: [this.data?.item.name_en, Validators.required],
      name_ar: [this.data?.item.name_ar, Validators.required],
  ...
  add() {
    ...
    if (this.type == 'Card') {
      this.cats.editcard(form, this.data?.item.id).subscribe(
        (res: any) => {
    ...
    if (this.type == 'Tap') {
      this.cats.edittap(form, this.data?.item.id).subscribe(
        (res: any) => {
          // console.log("success" ,res)
          if (res.status == true) {
            this.toastr.success(res.message, 'success', {
